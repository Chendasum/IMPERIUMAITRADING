...
class ProfessionalMomentumStrategy:
    ...
    def initialize(self, exchange_manager, risk_manager):
        ...
        for strength in self.momentum_levels.keys():
            self.momentum_performance['momentum_strength_performance'][strength] = {
                'signals': 0, 'successful': 0, 'avg_profit': 0.0
            }
        ...
        logger.info(f"📊 Multi-timeframe analysis: {[self.primary_timeframe] + self.confirmation_timeframes}")
        logger.info(f"🎯 Monitoring {len(self.asset_multipliers)} crypto assets")
    ...
    async def generate_signals(self):
        ...
        try:
            ...
            if not market_conditions['suitable_for_momentum']:
                logger.debug("📊 Market conditions not suitable for momentum trading")
                return signals
            ...
            for pair in active_pairs:
                try:
                    ...
                    momentum_analysis = await self._analyze_advanced_momentum(pair, market_data)
                    if not momentum_analysis:
                        continue
                    
                    # Generate signal if momentum is confirmed
                    signal = await self._create_momentum_signal(pair, momentum_analysis, market_conditions)
                    
                    if signal and await self._validate_momentum_signal(signal, market_data):
                        signals.append(signal)
                        self._track_signal_generation(signal)
                        logger.info(f"📈 Momentum Signal: {pair} - {signal['action']} - Strength: {signal['momentum_strength']} - Confidence: {signal['confidence']:.2%}")
    ...
    async def _assess_market_conditions(self):
        ...
        try:
            ...
            if not btc_data:
                return {'suitable_for_momentum': False, 'market_trend': 'unknown', 'volatility': 0}
            ...
            market_volatility = np.std(daily_returns) * np.sqrt(365)  # Annualized
            trend_strength = self._calculate_trend_strength(btc_closes)
            avg_volume = np.mean(btc_volumes[-7:])  # 7-day average
            
            # Determine market regime
            recent_momentum = (btc_closes[-1] - btc_closes[-7]) / btc_closes[-7]  # 7-day momentum
            ...
            suitable = (
                ...
                0.02 < market_volatility < 0.08 and  # 2-8% volatility
    ...
    async def _gather_momentum_data(self, pair):
        ...
        try:
            ...
            primary_data = await self._get_timeframe_data(pair, self.primary_timeframe, self.lookback_periods + 20)
            ...
            market_data['volume_analysis'] = self._analyze_volume_momentum(primary_data)
    ...
    def _analyze_volume_momentum(self, ohlcv_data):
        ...
        try:
            ...
            pv_correlation = np.corrcoef(price_changes, volume_changes)[0, 1] if len(price_changes) > 1 else 0
            ...
            return {
                ...
                'obv_trend': obv_trend,
                'volume_momentum_score': (volume_trend + abs(pv_correlation) + obv_trend) / 3
            }
            
        except Exception as e:
            logger.error(f"❌ Volume momentum analysis failed: {e}")
            return {'volume_trend': 0, 'price_volume_correlation': 0, 'obv_trend': 0, 'volume_momentum_score': 0}
    ...
    async def _analyze_advanced_momentum(self, pair, market_data):
        ...
        try:
            ...
            momentum_indicators = self._calculate_momentum_indicators(highs, lows, closes, volumes)
            
            # 2. Multi-timeframe momentum confirmation
            mtf_momentum = self._get_multitimeframe_momentum(market_data, current_price)
            ...
            # 5. Breakout analysis
            breakout_analysis = self._analyze_breakout_potential(highs, lows, closes, volumes)
            ...
            logger.error(f"❌ Advanced momentum analysis failed for {pair}: {e}")
            return None
    ...
    def _calculate_momentum_indicators(self, highs, lows, closes, volumes):
        ...
        try:
            ...
            if len(closes) > roc_period:
                indicators['roc'] = (closes[-1] - closes[-roc_period-1]) / closes[-roc_period-1]
            else:
                indicators['roc'] = 0
            ...
            indicators['momentum_oscillator'] = self._calculate_momentum_oscillator(closes)
    ...
    def _calculate_macd(self, prices):
        ...
        try:
            ...
            if len(macd_values) >= signal_period:
                macd_signal = self._calculate_ema(macd_values, signal_period)[-1]
            else:
    ...
    def _calculate_adx(self, highs, lows, closes, periods=14):
        ...
        try:
            ...
            di_minus = 100 * np.mean(dm_minus[-periods:]) / atr if atr > 0 else 0
    ...
    def _get_multitimeframe_momentum(self, market_data, current_price):
        ...
        try:
            ...
            for tf in self.confirmation_timeframes:
                ...
                if len(tf_closes) >= 10:
                    tf_momentum = (current_price - tf_closes[-10]) / tf_closes[-10]
                    ...
                    mtf_signals.append({
                        ...
                        'direction': 'bullish' if tf_momentum > 0.01 else 'bearish' if tf_momentum < -0.01 else 'neutral'
                    })
            
            # Calculate consensus
            bullish_signals = sum(1 for s in mtf_signals if s['direction'] == 'bullish')
            bearish_signals = sum(1 for s in mtf_signals if s['direction'] == 'bearish')
            ...
            return {'signals': [], 'consensus': 'neutral', 'consensus_strength': 0, 'total_confirmations': 0}
    ...
    def _calculate_trend_strength(self, prices):
        ...
        try:
            ...
            trend_strength = min(abs(slope) / np.mean(prices) * 100, 1.0) * r_squared
    ...
    def _analyze_trend_quality(self, closes, highs, lows):
        ...
        try:
            ...
            uptrend_quality = (higher_highs + higher_lows) / (2 * (len(recent_highs) - 1))
            ...
            downtrend_quality = (lower_highs + lower_lows) / (2 * (len(recent_highs) - 1))
            ...
            return {'uptrend_quality': 0, 'downtrend_quality': 0, 'trend_strength': 0, 'trend_consistency': 0}
    ...
    def _analyze_momentum_persistence(self, closes):
        ...
        try:
            ...
            # Persistence score
            if total_periods > 0:
                persistence_score = max(positive_momentum, negative_momentum) / total_periods
            else:
                persistence_score = 0
            ...
            return {'persistence_score': 0, 'momentum_periods': 0, 'positive_periods': 0, 'negative_periods': 0}
    ...
    def _analyze_breakout_potential(self, highs, lows, closes, volumes):
        ...
        try:
            ...
            return {'breakout_potential': 0, 'volume_confirmation': False, 'breakout_direction': 'neutral'}
    ...
    def _analyze_market_structure(self, highs, lows, closes):
        ...
        try:
            ...
            if len(swing_highs) >= 2 and len(swing_lows) >= 2:
                ...
                total_swings = max(len(swing_highs) - 1, len(swing_lows) - 1, 1)
            ...
            return {'structure_score': 0.5, 'structure_type': 'unknown', 'swing_highs': 0, 'swing_lows': 0}
    
    def _combine_momentum_analyses(self, pair, current_price, momentum_indicators, 
                                 mtf_momentum, trend_analysis, persistence_analysis,
                                 breakout_analysis, structure_analysis, volume_analysis):
        ...
        try:
            ...
            macd_histogram = momentum_indicators.get('macd', {}).get('histogram', 0)
            ...
            pair_threshold = self.base_momentum_threshold * self.asset_multipliers.get(pair, 1.0)
            
            # Bullish momentum conditions
            bullish_conditions = [
                rsi > 55 and rsi < 75,  # RSI in momentum zone but not overbought
            ...
            bearish_conditions = [
                ...
                roc < -pair_threshold,   # Rate of change below negative threshold
                cci < 0,                 # CCI negative
                adx > 25,                # Strong trend
            ...
            if bullish_score >= 0.6 and bullish_score > bearish_score:
                ...
                momentum_strength = self._categorize_momentum_strength(abs(roc))
            ...
            # Structure alignment
            if ((signal_direction == 'buy' and structure_analysis['structure_type'] == 'uptrend') or
                (signal_direction == 'sell' and structure_analysis['structure_type'] == 'downtrend')):
                confidence_multipliers.append(1.1)
    ...
    async def _create_momentum_signal(self, pair, momentum_analysis, market_conditions):
        ...
        try:
            ...
            atr = momentum_analysis['indicators'].get('atr', current_price * 0.02)
            ...
            return {
                ...
                'expected_holding_hours': self._estimate_holding_period(momentum_strength),
    ...
    async def _validate_momentum_signal(self, signal, market_data):
        ...
        try:
            ...
            recent_signals = [s for s in self.signal_history 
                            ...
                            (datetime.now() - s['timestamp']).total_seconds() < 3600]  # 1 hour
    ...
    def _rank_momentum_signals(self, signals):
        ...
        signals.sort(key=lambda s: (s.get('quality_score', 0) * 0.6 + s['confidence'] * 0.4), reverse=True)
    ...
    def _track_signal_generation(self, signal):
        ...
        if strength in self.momentum_performance['momentum_strength_performance']:
            self.momentum_performance['momentum_strength_performance'][strength]['signals'] += 1
    ...
    def record_momentum_result(self, signal, success, actual_profit=0):
        ...
        try:
            if success:
                ...
                success_count = self.momentum_performance['successful_momentum_trades']
                self.momentum_performance['avg_momentum_profit'] = ((current_avg * (success_count - 1)) + actual_profit) / success_count
                
                # Update strength-specific performance
                strength = signal.get('momentum_strength', 'moderate')
                if strength in self.momentum_performance['momentum_strength_performance']:
                    strength_perf = self.momentum_performance['momentum_strength_performance'][strength]
                    ...
                    strength_perf['avg_profit'] = ((current_strength_avg * (strength_success_count - 1)) + actual_profit) / strength_success_count
                
                logger.info(f"✅ Momentum success: {signal['pair']} - {actual_profit:.2%} profit")
            else:
    ...
    def get_strategy_performance(self):
        ...
        success_rate = (self.momentum_performance['successful_momentum_trades'] / 
        ...
        for strength, perf in self.momentum_performance['momentum_strength_performance'].items():
            if perf['signals'] > 0:
        ...
        return {
            ...
            'avg_profit': self.momentum_performance['avg_momentum_profit'],
            'best_momentum_strength': best_strength,
            'strength_breakdown': self.momentum_performance['momentum_strength_performance']
        }
