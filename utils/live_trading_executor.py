...
class ProfessionalLiveTradingExecutor:
    ...
    def __init__(self, exchange_manager):
        ...
        self.exchange_preferences = {
            'binance': {'priority': 1, 'fee_tier': 0.001},  # 0.1% fees
            'coinbase': {'priority': 2, 'fee_tier': 0.005}, # 0.5% fees
            'bybit': {'priority': 3, 'fee_tier': 0.001}     # 0.1% fees
        }
    ...
    async def initialize(self):
        ...
        try:
            ...
            self.execution_monitor_task = asyncio.create_task(self._monitor_executions())
    ...
    async def execute_smart_order(self, symbol: str, side: str, amount: float,
        ...
        try:
            ...
            if not await self._pre_execution_risk_check(symbol, side, amount, price):
                logger.error("❌ Pre-execution risk check failed")
                return None
            
            # Smart exchange selection
            best_exchange = await self._select_best_exchange(symbol, side, amount, order_type)
            ...
            execution_result = await self._execute_with_monitoring(
                best_exchange, symbol, side, amount, order_type, price, execution_id
            )
            
            if execution_result:
                # Post-execution processing
                await self._post_execution_processing(
                    execution_result, stop_loss, take_profit, strategy
                )
                ...
                logger.info(f"✅ Smart Order Executed: {execution_id} in {execution_time:.0f}ms")
    ...
    async def _pre_execution_risk_check(self, symbol: str, side: str, amount: float, 
        ...
        try:
            ...
            estimated_price = price if price else await self._get_market_price(symbol)
            ...
            if position_value > self.max_position_size:
                logger.error(f"❌ Position size ${position_value:.2f} exceeds limit ${self.max_position_size}")
                return False
            
            # Check total exposure
            current_exposure = self._calculate_total_exposure()
            if current_exposure + position_value > self.max_total_exposure:
                logger.error(f"❌ Total exposure would exceed limit: ${current_exposure + position_value:.2f}")
                return False
            ...
            if side == 'buy' and available_balance < required_margin:
                logger.error(f"❌ Insufficient balance: ${available_balance:.2f} < ${required_margin:.2f}")
                return False
    ...
    async def _select_best_exchange(self, symbol: str, side: str, amount: float, 
        ...
        try:
            ...
            for exchange_name in available_exchanges:
                try:
                    ...
                    prices = await self.exchange_manager.get_prices(exchange_name, [symbol])
                    ...
                    score = await self._calculate_exchange_score(
                        exchange_name, symbol, side, amount, price_data, order_type
                    )
            ...
            best_exchange = max(exchange_scores.keys(), key=lambda k: exchange_scores[k])
            logger.info(f"🎯 Selected exchange: {best_exchange} (score: {exchange_scores[best_exchange]:.2f})")
    ...
    async def _calculate_exchange_score(self, exchange_name: str, symbol: str, side: str,
                                      amount: float, price_data: Dict, order_type: OrderType) -> float:
        ...
        try:
            ...
            if volume > 0:
                market_impact = (amount * (ask if side == 'buy' else bid)) / volume
                impact_penalty = min(market_impact * 100, 50)  # Cap at 50 points
                score -= impact_penalty
            
            # Factor 4: Recent execution performance
            recent_performance = self._get_recent_exchange_performance(exchange_name)
    ...
    async def _execute_with_monitoring(self, exchange_name: str, symbol: str, side: str,
                                     amount: float, order_type: OrderType, price: Optional[float],
        ...
        try:
            ...
            pre_execution_price = await self._get_market_price(symbol, exchange_name)
            ...
            execution_price = order_result.get('average', order_result.get('price', 0))
            slippage = self._calculate_slippage(pre_execution_price, execution_price, side)
            market_impact = self._calculate_market_impact(amount, execution_price, symbol)
    ...
    async def _execute_market_order_with_monitoring(self, exchange, symbol: str, side: str,
                                                  amount: float, execution_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            try:
                order_result = await asyncio.wait_for(order_task, timeout=self.execution_timeout)
                logger.info(f"✅ Market order executed: {order_result.get('id', 'N/A')}")
                return order_result
                
            except asyncio.TimeoutError:
                logger.error(f"⏰ Market order timeout for execution {execution_id}")
                order_task.cancel()
                return None
    ...
    async def _execute_limit_order_with_monitoring(self, exchange, symbol: str, side: str,
                                                 amount: float, price: float, execution_id: str) -> Optional[Dict]:
        """Execute limit order with monitoring and partial fill handling"""
        try:
            # Place initial limit order
            order_result = await exchange.create_limit_order(symbol, side, amount, price)
            order_id = order_result.get('id')
            ...
            while time.time() - start_time < self.partial_fill_timeout:
                ...
                if filled_amount > 0:
                    logger.info(f"📊 Limit order partially filled: {filled_amount}/{amount}")
    ...
    async def _post_execution_processing(self, execution: TradeExecution,
        ...
        try:
            ...
            if stop_loss or take_profit:
                await self._set_position_protection(execution.symbol, stop_loss, take_profit)
            ...
            logger.info(f"✅ Post-execution processing completed for {execution.execution_id}")
    ...
    async def _check_position_exits(self, position: Position):
        ...
        try:
            ...
            if should_close:
                logger.info(f"🎯 {exit_reason} triggered for {position.symbol} at {current_price}")
    ...
    def _calculate_slippage(self, expected_price: float, execution_price: float, side: str) -> float:
    ...
    def _calculate_market_impact(self, amount: float, price: float, symbol: str) -> float:
        """Calculate market impact of the trade"""
        # Simplified market impact calculation
        trade_value = amount * price
        return min(trade_value / 1000000, 0.01)  # Max 1% impact

    def _calculate_realized_pnl(self, position: Position, execution: TradeExecution) -> float:
    ...
    async def _get_market_price(self, symbol: str, exchange_name: Optional[str] = None) -> Optional[float]:
        ...
        try:
            ...
            for exchange in exchanges:
                prices = await self.exchange_manager.get_prices(exchange, [symbol])
                if prices and symbol in prices:
                    price_data = prices[symbol]
                    return (price_data.get('bid', 0) + price_data.get('ask', 0)) / 2
    ...
    async def _get_available_balance(self) -> float:
        ...
        try:
            ...
            primary_exchange = self.exchange_manager.get_connected_exchanges()[0]
            balance = await self.exchange_manager.get_account_balance(primary_exchange)
    ...
    def _check_position_concentration(self, symbol: str, position_value: float) -> bool:
        ...
        if total_exposure > 0:
            ...
            if concentration > 0.3:
                logger.error(f"❌ Position concentration too high: {concentration:.1%}")
                return False
        return True
    ...
    async def _check_risk_limits(self):
        ...
        try:
            ...
            if total_exposure > self.max_total_exposure:
                logger.warning(f"⚠️ Total exposure limit exceeded: ${total_exposure:.2f}")
                # Could implement automatic position reduction here
            
            # Check daily losses
            if self._check_daily_loss_limit():
                logger.error("🛑 Daily loss limit reached - activating emergency stop")
                self.emergency_stop = True
            
        except Exception as e:
            logger.error(f"❌ Risk limit check failed: {e}")

    def _update_execution_metrics(self, execution: TradeExecution, execution_time_ms: float):
        ...
        self.execution_metrics['avg_execution_time'] = ((current_avg_time * (total - 1)) + execution_time_ms) / total
        
        # Slippage
        current_avg_slippage = self.execution_metrics['avg_slippage']
        self.execution_metrics['avg_slippage'] = ((current_avg_slippage * (total - 1)) + execution.slippage) / total
    ...
    def _get_recent_exchange_performance(self, exchange_name: str) -> float:
        ...
        success_rate = len([e for e in recent_executions if e.status == OrderStatus.FILLED]) / len(recent_executions)
        avg_slippage = np.mean([e.slippage for e in recent_executions])
        
        # Combine metrics into performance score
        slippage_penalty = min(abs(avg_slippage) * 10, 0.5)  # Cap penalty at 0.5
    ...
    async def _load_existing_positions(self):
        ...
        try:
            ...
            for exchange_name in self.exchange_manager.get_connected_exchanges():
                try:
                    # Get open orders
                    open_orders = await self.exchange_manager.exchanges[exchange_name].fetch_open_orders()
                    
                    # Get current positions (for margin trading)
                    # This would depend on exchange capabilities
                    
                    logger.debug(f"📊 Loaded {len(open_orders)} open orders from {exchange_name}")
                    
                except Exception as e:
                    logger.warning(f"⚠️ Could not load positions from {exchange_name}: {e}")
        ...
        try:
            while True:
                try:
                    ...
                    execution_item = await asyncio.wait_for(self.execution_queue.get(), timeout=1.0)
                    
                    # Check for timeouts
                    if time.time() - execution_item['start_time'] > self.execution_timeout:
                        logger.warning(f"⏰ Execution timeout detected: {execution_item['execution_id']}")
    ...
    async def _set_position_protection(self, symbol: str, stop_loss: Optional[float], 
    ...
    async def execute_market_order(self, exchange_name: str, symbol: str, side: str, amount: float) -> Optional[Dict]:
    ...
    async def get_open_orders(self) -> Dict[str, List]:
        ...
        for exchange_name in self.exchange_manager.get_connected_exchanges():
            try:
                ...
                logger.error(f"❌ Failed to get {exchange_name} open orders: {e}")
    ...
    def get_positions_summary(self) -> Dict[str, Any]:
        ...
        total_realized_pnl = sum(pos.realized_pnl for pos in self.open_positions.values())
        total_exposure = self._calculate_total_exposure()
        ...
        for pos in self.open_positions.values():
            position_details.append({
                ...
                'unrealized_pnl_pct': (pos.unrealized_pnl / (pos.size * pos.entry_price)) * 100,
                'max_profit': pos.max_profit,
                'max_drawdown': pos.max_drawdown,
                'entry_time': pos.entry_timestamp.isoformat(),
                'holding_period': str(datetime.now() - pos.entry_timestamp)
            })
    ...
    def get_execution_summary(self) -> Dict[str, Any]:
        ...
        recent_executions = self.trade_history[-50:] if len(self.trade_history) > 50 else self.trade_history
        ...
        successful_executions = [e for e in recent_executions if e.status == OrderStatus.FILLED]
    ...
    def get_risk_metrics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'exposure_utilization': (self._calculate_total_exposure() / self.max_total_exposure) * 100,
            'position_count': len(self.open_positions),
            'max_positions': 10,  # Could be configurable
            'largest_position': max([pos.size * pos.current_price for pos in self.open_positions.values()]) if self.open_positions else 0
        }
    ...
    def get_trade_summary(self) -> Dict[str, Any]:
        ...
        return {
            'total_trades': execution_summary['total_executions'],
            'executed_trades': execution_summary['successful_executions'],
            'pending_trades': len(self.open_orders),
            'open_positions': positions_summary['total_positions'],
            'total_pnl': positions_summary['total_unrealized_pnl'] + positions_summary['total_realized_pnl'],
            'latest_trades': execution_summary['recent_executions'][-5:]
        }
